...
from apps.core.models import GeneratedAsset, GenerationJob, Order, OrderPhoto
...
class GenerationService:
    ...
    def generate_preview(self, *, order: Order) -> GeneratedAsset:
        job = self._start_preview_job(order=order)
        try:
            request = self._build_preview_request(job=job)
            result = self.provider.generate_preview(request)
            if not result.content:
                raise GenerationError("Image provider returned empty content")
            return self._complete_preview(job=job, result=result)
    ...
    @transaction.atomic
    def _start_preview_job(self, *, order: Order) -> GenerationJob:
        locked_order = (
    ...
    def _build_preview_request(self, *, job: GenerationJob) -> ImageGenerationRequest:
        order = (
    ...
    @transaction.atomic
    def _complete_preview(self, *, job: GenerationJob, result) -> GeneratedAsset:
        ...
        if locked_order.status != Order.Status.PREVIEW_GENERATING:
            raise GenerationError(
                f"Order #{locked_order.pk} is not generating preview: {locked_order.status}"
            )
