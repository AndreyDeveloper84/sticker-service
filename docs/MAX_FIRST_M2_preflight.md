...
| 15 | Отправить набор | оператор | «Отправить набор клиенту» (`core_order_deliver_final`; кнопка есть только при открытом QC gate) | `DELIVERY_IN_PROGRESS` → клиент получил 1 файл (MAX `send_file`: вложение `file`, PNG с альфой, имя `sticker-<slot>.png`; при `attachment.not.ready` отправка повторяется с тем же token) + «набор готов» → `DELIVERED`; `FinalDelivery attempt=1`, slot `sent` с message_id | **FinalDelivery id**, per‑slot **message id** |
