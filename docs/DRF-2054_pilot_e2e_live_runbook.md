...
| 14 | «Вернуть звёзды» (T-ячейки, после проверки) | суперпользователь | карточка заказа → блок «Оплата» → «Вернуть звёзды» → confirm-страница с суммой «100/460 XTR» и обязательной причиной → `refundStarPayment` через пул → `Payment.status=REFUNDED`, `metadata.refund` (charge_id, refunded_at, actor_ref, reason), `OrderEvent payment.refunded`; повторный возврат отказывается до вызова провайдера; статус заказа **не меняется** (решение оператора отдельно); в «Экономике заказа» — «Выручка: 0 XTR — возврат (возвращено: 100 XTR)», в «Метриках Pilot» — строка «Возвращено (в выручку не входит)» |
| 13b | Набор стикеров (DRF-2163) | бот/оператор | Telegram: после «Доставлен» бот создаёт набор клиента `sticks_<user_id>_by_<bot>` (createNewStickerSet / addStickerToSet, PNG ≤512 KB, emoji по эмоции) и шлёт ссылку t.me/addstickers/…; запись в `FinalDelivery.summary.sticker_set` (set_name, stickers slot→file_id, link_message_id, status); сбой Bot API → в блоке «Доставка» строка «сбой — …» и кнопка «Создать набор повторно» (добавляет только недостающие слоты, ссылка не дублируется). MAX: итоговое сообщение содержит инструкцию «Стикеры в MAX» |
