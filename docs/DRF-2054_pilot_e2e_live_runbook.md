...
| 13 | «Отправить набор клиенту» (DRF-2053) | оператор | кнопка появляется только при открытом QC gate; `DELIVERY_IN_PROGRESS` («Доставка») → клиент получает 9/1 **нормализованных** 512 px файлов (TG `sendDocument`, PNG с прозрачностью; MAX `send_image`) в порядке `selection.emotions` + сообщение «набор готов» → `DELIVERED`; `FinalDelivery attempt=1`, все slots `sent` с message_id. «Продолжить доставку» (главная кнопка при статусе «Доставка», по 3 стикера за нажатие) до «Доставлен»; после `DELIVERED` — «Готово», повторных отправок нет |
