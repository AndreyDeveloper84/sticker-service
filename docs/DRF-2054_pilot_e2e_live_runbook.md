...
Статус: **подготовка** (engineering E2E зелёный; live-прогон не выполнялся).
...
1. `origin/dev` задеплоен на staging и содержит: DRF-2050, 2057, 2051, 2052 (dev
   `cf4864b` и выше) **и DRF-2053 (PR #35)** — без #35 delivery-стадия недоступна,
   матрицу можно догнать только до `READY_FOR_DELIVERY`.
...
8. Известный риск (DRF-2052, комментарий в Linear): `OpenAIImageProvider._generate`
   вызывает `images.edit` без `size`/`background`. Если FULL-вывод не 512 px / без
   alpha, автоматический QC даст `bad_dimensions` / `missing_alpha` и заблокирует
   delivery (gate G6 в `tests_e2e_pilot_gates.py`). Это ожидаемое поведение gate,
   а не дефект QC; фиксируется как BLOCKED c кодами причин.
...
| 5 | «Фото загружены» | клиент | `READY_FOR_CHECKOUT`; summary: «Стикеров: 9/1», «Цена: 460/100 Stars» (TG) или «500/100 ₽» (MAX); MAX сразу получает кнопку «Оплатить заказ» с URL YooKassa и Order → `AWAITING_PAYMENT` |
| 6 | Оплата | клиент | TG: invoice `amount_stars` 460/100, pre_checkout ok, `successful_payment`; MAX: webhook `payment.succeeded`. Итог: `Payment.CONFIRMED` (`telegram_stars/XTR` или `yookassa/RUB`), Order `PAID`, ровно один CONFIRMED |
...
| 9b | (вариант) «Нужно исправить» → категория | клиент | `REVISION_REQUESTED`, `Revision` ×1; **в консоли нет действия для revision-генерации** — оператор выполняет `GenerationService(provider=OpenAIImageProvider()).generate_revision(order=order)` в `manage.py shell` → `INTERNAL_PREVIEW_REVIEW` → шаг 8 → 9a. Вторая правка должна быть отклонена (409) |
| 10 | Start / resume full production ×N | оператор | по одному slot за нажатие; после 9-го/1-го — `QUALITY_CONTROL`; `GenerationJob(full)` ×9/×1 SUCCEEDED, `input_metadata.source_preview_id` = одобренное превью; повторное нажатие после завершения — сообщение об ошибке, новых jobs нет |
| 11 | Открыть QC report | оператор | `QcReport attempt=1`, `expected_count` 9/1, automated checks: смотреть `dimensions`/`alpha_channel`/`file_size` (см. §3 п.8) |
...
| 13 | Deliver final set (PR #35) | оператор | `DELIVERY_IN_PROGRESS` → клиент получает 9/1 файлов (TG `sendDocument`, PNG с прозрачностью; MAX `send_image`) в порядке `selection.emotions` + сообщение «набор готов» → `DELIVERED`; `FinalDelivery attempt=1`, все slots `sent` с message_id. Resume после `DELIVERED` — отказ, повторных отправок нет |
...
- QcReport attempts с `automated_checks` и `reason_codes`.
...
## 7. Metrics handoff (DRF-2055, PR #34)

После merge #34 сразу после live-прогона снять снапшот за окно прогона:
...
1. Нет console-действия для revision-генерации (`REVISION_REQUESTED → INTERNAL_PREVIEW_REVIEW`);
   live-вариант «1 revision» требует shell. Кандидат на follow-up ticket.
2. Формат FULL-вывода (512 px / alpha) — см. §3 п.8; решение (параметры провайдера
   или post-processing) принимается по результату первого live FULL.
3. Delivery-стадия зависит от merge PR #35; до него live-матрица заканчивается на
   `READY_FOR_DELIVERY` и не является acceptance.
